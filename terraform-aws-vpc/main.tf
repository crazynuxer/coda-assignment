...
module "flowlogs_to_s3_naming" {
  source = "git@github.com:crazynuxer/terraform-aws-resource-naming.git?ref=v0.8.0"
  name_prefix = format(
    "%s-flowlogs-%s",
    aws_vpc.this.id,
...
resource "aws_s3_bucket_logging" "flowlogs_to_s3_logging" {
  bucket        = aws_s3_bucket.flowlogs_to_s3.id
  target_bucket = aws_s3_bucket.flowlogs_to_s3.id
  target_prefix = "${module.flowlogs_to_s3_naming.name}/"
}

resource "aws_s3_bucket_lifecycle_configuration" "flowlogs_to_s3_lifecycle" {
  bucket = aws_s3_bucket.flowlogs_to_s3.id
...
/*
resource "aws_s3_bucket_acl" "flowlogs_to_s3_acl" {
  bucket = aws_s3_bucket.flowlogs_to_s3.id
  acl    = "private"
}
*/
resource "aws_s3_bucket_server_side_encryption_configuration" "flowlogs_to_s3_encryption" {
  bucket = aws_s3_bucket.flowlogs_to_s3.id
